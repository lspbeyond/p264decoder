...
static inline int p264_tapfilter( uint8_t *pix, int i_pix_next )
{
    return pix[-2*i_pix_next] - 5*pix[-1*i_pix_next] + 20*(pix[0] + pix[1*i_pix_next]) - 5*pix[ 2*i_pix_next] + pix[ 3*i_pix_next];
}
static inline int p264_tapfilter1( uint8_t *pix )
{
...
#define op_scale2(x) dst[x] = p264_clip_uint8( (dst[x]*i_weight1 + src[x]*i_weight2 + (1<<5)) >> 6 )
...
static inline void mc_hh( uint8_t *src, int i_src_stride, uint8_t *dst, int i_dst_stride, int i_width, int i_height )
{
    ...
    for( y = 0; y < i_height; y++ )
    {
        for( x = 0; x < i_width; x++ )
        {
            dst[x] = p264_mc_clip1( ( p264_tapfilter1( &src[x] ) + 16 ) >> 5 );
        }
        src += i_src_stride;
        dst += i_dst_stride;
    }
...
static inline void mc_hv( uint8_t *src, int i_src_stride, uint8_t *dst, int i_dst_stride, int i_width, int i_height )
{
    ...
    for( y = 0; y < i_height; y++ )
    {
        for( x = 0; x < i_width; x++ )
        {
            dst[x] = p264_mc_clip1( ( p264_tapfilter( &src[x], i_src_stride ) + 16 ) >> 5 );
        }
        src += i_src_stride;
        dst += i_dst_stride;
    }
...
static inline void mc_hc( uint8_t *src, int i_src_stride, uint8_t *dst, int i_dst_stride, int i_width, int i_height )
{
    ...
    for( x = 0; x < i_width; x++ )
    {
        ...
        tap[0] = p264_tapfilter1( &pix[-2*i_src_stride] );
        tap[1] = p264_tapfilter1( &pix[-1*i_src_stride] );
        tap[2] = p264_tapfilter1( &pix[ 0*i_src_stride] );
        tap[3] = p264_tapfilter1( &pix[ 1*i_src_stride] );
        tap[4] = p264_tapfilter1( &pix[ 2*i_src_stride] );

        for( y = 0; y < i_height; y++ )
        {
            tap[5] = p264_tapfilter1( &pix[ 3*i_src_stride] );

            *out = p264_mc_clip1( ( tap[0] - 5*tap[1] + 20 * tap[2] + 20 * tap[3] -5*tap[4] + tap[5] + 512 ) >> 10 );
                                        ...
                                        int i_width, int i_height )
{
    if (i_width == 2) {
        ...
        p264_mc_chroma_sse(src, i_src_stride, dst, i_dst_stride,
                              d8x, d8y, i_height, i_width);
    }
}
#endif

void p264_mc_init( int cpu, p264_mc_functions_t *pf )
{
    pf->mc_luma   = mc_luma;
    pf->get_ref   = get_ref;
    ...
    if( cpu&P264_CPU_MMXEXT ) {
        p264_mc_mmxext_init( pf );
        pf->mc_chroma = motion_compensation_chroma_sse;
    }
#endif
#ifdef HAVE_SSE2
    if( cpu&P264_CPU_SSE2 )
        p264_mc_sse2_init( pf );
#endif
#ifdef ARCH_PPC
    if( cpu&P264_CPU_ALTIVEC )
        p264_mc_altivec_init( pf );
#endif
}

extern void p264_horizontal_filter_mmxext( uint8_t *dst, int i_dst_stride,
                                           uint8_t *src, int i_src_stride,
                                           int i_width, int i_height );
extern void p264_center_filter_mmxext( uint8_t *dst1, int i_dst1_stride,
...
void p264_frame_filter( int cpu, p264_frame_t *frame )
{
    ...
    if ( cpu & P264_CPU_MMXEXT )
    {
        p264_horizontal_filter_mmxext(frame->filtered[1] - 8 * stride - 8, stride,
            frame->plane[0] - 8 * stride - 8, stride,
            stride - 48, frame->i_lines[0] + 16);
        p264_center_filter_mmxext(frame->filtered[2] - 8 * stride - 8, stride,
...
void p264_frame_init_lowres( int cpu, p264_frame_t *frame )
{
    ...
    p264_frame_expand_border_lowres( frame );
}

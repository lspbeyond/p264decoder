...
/* {{{ [fold] void p264_visualize_init( p264_t *h ) */
void p264_visualize_init( p264_t *h )
{
    int mb = h->sps->i_mb_width * h->sps->i_mb_height;
    h->visualize = p264_malloc(mb * sizeof(visualize_t));
}
/* }}} */
/* {{{ [fold] void p264_visualize_mb( p264_t *h ) */
void p264_visualize_mb( p264_t *h )
{
    ...
    for (y=0; y<4; y++) for (x=0; x<4; x++)
        v->intra4x4_pred_mode[y][x] = h->mb.cache.intra4x4_pred_mode[P264_SCAN8_0+y*8+x];
    for (l=0; l<2; l++) for (y=0; y<4; y++) for (x=0; x<4; x++) {
        for (i=0; i<2; i++) {
            v->mv[l][y][x][i] = h->mb.cache.mv[l][P264_SCAN8_0+y*8+x][i];
        }
        v->ref[l][y][x] = h->mb.cache.ref[l][P264_SCAN8_0+y*8+x];
    }
    v->i_intra16x16_pred_mode = h->mb.i_intra16x16_pred_mode;
}
/* }}} */
/* {{{ [fold] void p264_visualize_close( p264_t *h ) */
void p264_visualize_close( p264_t *h )
{
    p264_free(h->visualize);
}
/* }}} */
/* {{{ [fold] void p264_visualize_show( p264_t *h ) */
        ...
        if (v->i_type==P_L0 || v->i_type==P_8x8 || v->i_type==P_SKIP) {
            ...
            if (v->i_partition==D_8x8) {
                for (i=0; i<2; i++) for (j=0; j<2; j++) {
                    ...
                    l = p264_mb_partition_listX_table[0][sp] ? 0 : 1; /* FIXME: not tested if this works */
                    if (IS_SUB8x8(sp)) {

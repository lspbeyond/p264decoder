...
int p264_cqm_parse_jmlist( p264_t *h, const char *buf, const char *name,
                           uint8_t *cqm, const uint8_t *jvt, int length )
{
    ...
    for( i = 0; i < length && (p = strpbrk( p, " \t\n," )) && (p = strpbrk( p, "0123456789" )); i++ )
    {
        ...
        if( coef < 1 || coef > 255 )
        {
            p264_log( h, P264_LOG_ERROR, "bad coefficient in list '%s'\n", name );
            return -1;
        }
        cqm[i] = coef;
    }
    ...
    {
        p264_log( h, P264_LOG_ERROR, "not enough coefficients in list '%s'\n", name );
        return -1;
    }
...
int p264_cqm_parse_file( p264_t *h, const char *filename )
{
    char *buf, *p;
    int b_error = 0;

    h->param.i_cqm_preset = P264_CQM_CUSTOM;
    
    buf = p264_slurp_file( filename );
    if( !buf )
    {
        p264_log( h, P264_LOG_ERROR, "can't open file '%s'\n", filename );
        return -1;
    }
    ...
    b_error |= p264_cqm_parse_jmlist( h, buf, "INTRA4X4_LUMA",   h->param.cqm_4iy, p264_cqm_jvt4i, 16 );
    b_error |= p264_cqm_parse_jmlist( h, buf, "INTRA4X4_CHROMA", h->param.cqm_4ic, p264_cqm_jvt4i, 16 );
    b_error |= p264_cqm_parse_jmlist( h, buf, "INTER4X4_LUMA",   h->param.cqm_4py, p264_cqm_jvt4p, 16 );
    b_error |= p264_cqm_parse_jmlist( h, buf, "INTER4X4_CHROMA", h->param.cqm_4pc, p264_cqm_jvt4p, 16 );
    b_error |= p264_cqm_parse_jmlist( h, buf, "INTRA8X8_LUMA",   h->param.cqm_8iy, p264_cqm_jvt8i, 64 );
    b_error |= p264_cqm_parse_jmlist( h, buf, "INTER8X8_LUMA",   h->param.cqm_8py, p264_cqm_jvt8p, 64 );

 ...
 * $Id: core.c,v 1.1 2004/06/03 19:27:06 fenrir Exp $
 *
...
static void p264_log_default( void *, int, const char *, va_list );

/****************************************************************************
 * p264_param_default:
 ****************************************************************************/
void    p264_param_default( p264_param_t *param )
{
    /* */
    memset( param, 0, sizeof( p264_param_t ) );

    /* CPU autodetect */
    param->cpu = p264_cpu_detect();
    param->i_threads = 1;

    /* Video properties */
    param->i_csp           = P264_CSP_I420;
    param->i_width         = 0;
    param->i_height        = 0;
    ...
    param->rc.psz_stat_out = "p264_2pass.log";
    param->rc.b_stat_read = 0;
    param->rc.psz_stat_in = "p264_2pass.log";
    ...
    /* Log */
    param->pf_log = p264_log_default;
    param->p_log_private = NULL;
    param->i_log_level = P264_LOG_INFO;

    /* */
    param->analyse.intra = P264_ANALYSE_I4x4 | P264_ANALYSE_I8x8;
    param->analyse.inter = P264_ANALYSE_I4x4 | P264_ANALYSE_I8x8
                         | P264_ANALYSE_PSUB16x16 | P264_ANALYSE_BSUB16x16;
    param->analyse.i_direct_mv_pred = P264_DIRECT_PRED_TEMPORAL;
    param->analyse.i_me_method = P264_ME_HEX;
    ...
    param->i_cqm_preset = P264_CQM_FLAT;
...
static void p264_log_default( void *p_unused, int i_level, const char *psz_fmt, va_list arg )
{
    char *psz_prefix;
    switch( i_level )
    {
    ...
    fprintf( stderr, "p264 [%s]: ", psz_prefix );
...
{
    pic->i_type = P264_TYPE_AUTO;
    pic->i_qpplus1 = 0;
    //lsp
    pic->i_width=i_width;
    pic->i_height=i_height;
    //lsp end
    pic->img.i_csp = i_csp;
    switch( i_csp & P264_CSP_MASK )
    {
        case P264_CSP_I420:
        case P264_CSP_YV12:
            pic->img.i_plane = 3;
            pic->img.plane[0] = p264_malloc( 3 * i_width * i_height / 2 );
        ...
        case P264_CSP_I422:
            pic->img.i_plane = 3;
            pic->img.plane[0] = p264_malloc( 2 * i_width * i_height );
            pic->img.plane[1] = pic->img.plane[0] + i_width * i_height;
        ...
        case P264_CSP_I444:
            pic->img.i_plane = 3;
            pic->img.plane[0] = p264_malloc( 3 * i_width * i_height );
            pic->img.plane[1] = pic->img.plane[0] + i_width * i_height;
        ...
        case P264_CSP_BGRA:
            pic->img.i_plane = 1;
            pic->img.plane[0] = p264_malloc( 4 * i_width * i_height );
...
{
    p264_free( pic->img.plane[0] );

    /* just to be safe */
    memset( pic, 0, sizeof( p264_picture_t ) );
}

/****************************************************************************
 * p264_nal_encode:
 ****************************************************************************/
int p264_nal_encode( void *p_data, int *pi_data, int b_annexeb, p264_nal_t *nal )
{
    uint8_t *dst = p_data;
 ...
 * p264_nal_decode:
 ****************************************************************************/
int p264_nal_decode( p264_nal_t *nal, void *p_data, int i_data )
{
    uint8_t *src = p_data;
...
char *p264_param2string( p264_param_t *p, int b_res )
{
    ...
    s += sprintf( s, " me=%s", p264_motion_est_names[ p->analyse.i_me_method ] );

...
static int  vlc_table_realloc( p264_vlc_table_t *table, int i_size )
{
    int i_index;
    ...
    table->lookup = p264_realloc( table->lookup, sizeof( vlc_lookup_t ) * table->i_lookup );

    return( i_index );
}

static int vlc_table_create_part( p264_vlc_table_t *table, const vlc_t *vlc, int i_lookup_bits, int i_nb_vlc, int i_prefix_code, int i_prefix_length )
{
    int i;
    int i_nb_lookup;
    vlc_lookup_t *lookup;
...
p264_vlc_table_t *p264_vlc_table_lookup_new( const vlc_t *vlc, int i_vlc, int i_lookup_bits )
{
    p264_vlc_table_t *table = p264_malloc( sizeof( p264_vlc_table_t ) );
...
void p264_vlc_table_lookup_delete( p264_vlc_table_t *table )
{
    p264_free( table->lookup );
    p264_free( table );
}

#if 0
void p264_vlc_table_lookup_print( p264_vlc_table_t *table )
{
    int idx;
...
int main(void)
{
    ...
    /* p264_coeff_token */
    fprintf( stderr, "static const vlc_lookup_t p264_coeff_token_lookup[5][]=\n{\n" );
    for( i = 0; i < 5; i++ )
    {
        fprintf( stderr, "    {\n" );
        table = p264_vlc_table_lookup_new( p264_coeff_token[i], 17*4, 6 );
        p264_vlc_table_lookup_print( table );
        p264_vlc_table_lookup_delete( table );
        fprintf( stderr, "    },\n" );
    }
    fprintf( stderr, "};\n" );
    ...
    vlce = convert_vlc_to_vlce( p264_level_prefix, 16 );
    do_vlc_table_create( vlce, 16, "p264_level_prefix_lookup", 8 );
    free( vlce );

    for( i_table = 0; i_table < 15; i_table++ )
    {
        char name[512];
        vlce = convert_vlc_to_vlce( p264_total_zeros[i_table], 16 );
        sprintf( name, "p264_total_zeros_%d", i_table );
    ...
    for( i_table = 0; i_table < 3; i_table++ )
    {
        ...
        vlce = convert_vlc_to_vlce( p264_total_zeros_dc[i_table], 4 );
        sprintf( name, "p264_total_zeros_dc_%d", i_table );
    ...
    {
        char name[512];
        vlce = convert_vlc_to_vlce( p264_run_before[i_table], 15 );
        sprintf( name, "p264_run_before_%d", i_table );

...
int p264_sps_read( bs_t *s, p264_sps_t sps_array[32] )
{
    p264_sps_t *sps;
    ...
    fprintf( stderr, "p264_sps_read: sps:0x%x profile:%d/%d poc:%d ref:%d %xx%d crop:%d-%d-%d-%d\n",
             sps->i_id,
...
/* return -1 if invalid, else the id */
int p264_pps_read( bs_t *s, p264_pps_t pps_array[256] )
{
    p264_pps_t *pps;
    int id;
    int i;
    ...
    fprintf( stderr, "p264_sps_read: pps:0x%x sps:0x%x %s slice_groups=%d ref0:%d ref1:%d QP:%d QS:%d QC=%d DFC:%d CIP:%d RPC:%d\n",
    ...
    //lsp?? //temp way
    for( i = 0; i < 6; i++ )
      pps->scaling_list[i] = p264_cqm_flat16;
    //lsp
